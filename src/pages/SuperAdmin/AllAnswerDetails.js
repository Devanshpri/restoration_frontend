...
const AllAnswerDetails = () => {
  ...
  useEffect(() => {
    ...
    // })()
    (async () => {
      const currentId = await localStorage.getItem("AnswerOfCurrClick");
      await axios
        .get(
          `https://restorationbackend.herokuapp.com/api/users/${currentId}/getAnswers`
        )
        .then((res) => {
  ...
  records.forEach(async (record, i) => {
    ...
    record.option12 = answersOfUser[i]?.sub_question_answer
      ? answersOfUser[i].sub_question_answer
      : "Not answered";
  });
  // console.log(records);
  ...
  // const linkFormatter = () => {
  //   return <a href="/superadmin/answerstoquestionnaire" >More</a>;
  // };
  const columns = [
    {
      text: "S. No.",
      ...
      headerStyle: (colum, colIndex) => {
        return { whiteSpace: "nowrap", width: "70px" };
      },
      sort: true,
    },
    {
    ...
    },
    {
      ...
      headerStyle: (colum, colIndex) => {
        return { whiteSpace: "nowrap", width: "300px" };
      },
      sort: true,
    },
    {
    ...
    <React.Fragment>
      <Row className="page-title">
        <Col className="col-12">
          <PageTitle breadCrumbItems={[]} title={"All Answer "} />
        </Col>
      </Row>
      <Row>
...
export default AllAnswerDetails;
